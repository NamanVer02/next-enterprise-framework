...
async function startPocketBase() {
  ...
  try {
    ...
    console.error(
      "Failed to start PocketBase. Please start it manually and try again."
    );
    return false;
  } catch (error) {
    console.error("Failed to start PocketBase:", error.message);
    console.log("Please start PocketBase manually:");
    console.log("- Windows: pocketbase-server\\pocketbase.exe serve");
    console.log("- macOS/Linux: ./pocketbase-server/pocketbase serve");
    return false;
  }
}
...
async function createCollections() {
  ...
  try {
    ...
    console.log(
      "- tagline: Experience the perfect fusion of luxury and performance"
    );
...
async function main() {
  try {
    ...
    if (!pbRunning) {
      console.log("");
      console.log(
        "Please start PocketBase manually and run this script again."
      );
      process.exit(1);
    }
    ...
    console.log("");
    console.log(
      "Setup instructions displayed. After completing these steps, your dashboard will be connected to PocketBase."
    );
    console.log(
      "Visit http://localhost:3000/dashboard to see your car model data."
    );
  } catch (error) {
